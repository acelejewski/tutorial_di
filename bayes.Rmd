...
Nevertheless the idea that there if subjectivity in selecting a priror, and by extension subjectivity surrounding can be seen as a constrast to to NHSTs. However, interpration of data with NHSTs does preclude the fact the same evidence may be viewed in a different light. 

The choice o prior is not limited to the normal distribution. 

###Normal distributions with know variance
Depending on the complexity of the model, the distribution of parameters and the number of parameters, the integral my have know solution or not. if there is no known solution, Monte Carlo methods can be applied. 

If the prior and and likelihood are both normal, the posterior is also normal. Provided the variance of the sample is known, there is a simple analytic solution. if the variance is unknown, an analytical solution exists using the t distribution. 

The likelihood function for a same from a normal distribution with known variance can be expressed as:
$$\ell (\theta|D) \sim N( \hat{\mu}, \hat{\sigma}_\hat{\mu} )$$
If the prior elicited for the analysis is a normal distribution with mean $\mu_{prior}$ and a standard deviation $\sigma_{prior}$ then the posterior will als be normal, its mean and SD is a funtion of $\hat{\mu}$, $\mu_{prior}$ and $\sigma^2_{prior}$. The variance of a the posterior distriubiton is. 
